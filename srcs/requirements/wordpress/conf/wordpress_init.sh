...
if [ ! -f "/var/www/wp-config.php" ]; then
	...
	php83 wp-cli.phar config create --dbname=$DB_NAME --dbuser=$ROOT_USER --dbpass=$DB_ROOT_PASSWORD --dbhost=mariadb:3306 --path='/var/www/wordpress' --allow-root
	echo -e "${green}config created${reset}"

	echo -e "${grey}config installation${reset}"
	php83 wp-cli.phar core install --url=$DOMAIN_NAME --title=inception --admin_user=$ROOT_USER --admin_password=$DB_ROOT_PASSWORD --admin_email=test@test.com --path='/var/www/wordpress' --allow-root
	echo -e "${green}config installed${reset}"

	php83 wp-cli.phar user create --role=author $USER $USER_MAIL --user_pass=$WP_PASSWORD
